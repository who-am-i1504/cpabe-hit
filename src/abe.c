...
int keygen(prv_sk ** prv, msk_p *msk, pub_p * pp, char ** attributes)
{
    *prv = malloc(sizeof(prv_sk));
    ...
    // 计算k0
    element_mul_zn((*prv) -> k0, (*prv) -> k0, (*prv) -> k1);
    // 计算k1
    ...
    GPtrArray * attr_list;
    attr_list = g_ptr_array_new();
    while (*attributes)
    {
        char *attr = strdup(*(attributes++));
        g_ptr_array_add(attr_list, attr);
        element_t r_i;
        element_t A_i;
        ...
        element_init_G1(mid, pp -> p);
        element_init_Zr(r_i, pp -> p);
        element_init_Zr(A_i, pp -> p);
        element_init_G1(*k2i, pp -> p);
        element_init_G1(*k3i, pp -> p);
        
        char *hash_code;
        hash(&hash_code, attr);
        // printf("%s \t %s \t",attr, hash_code);
        element_random(r_i);
        element_from_hash(A_i, hash_code, strlen(hash_code));
        // element_printf("\t%B\n", A_i);
        free(hash_code);
        ...
        element_pow_zn(mid, pp -> u, A_i);
        element_mul(mid, mid, pp -> h);
        element_pow_zn(mid, mid, r_i);

        element_pow_zn(*k3i, pp -> v, neg_r);
        element_mul(*k3i, mid, *k3i);
    ...
    // construct attributes as list;
    (*prv) -> attribute = malloc(sizeof(char *) * (attr_list->len + 1));
    for (int i = 0; i < attr_list -> len; i ++ )
        *((*prv) -> attribute + i) = attr_list -> pdata[i];
    *((*prv) -> attribute + attr_list -> len) = NULL;
    g_ptr_array_free(attr_list, FALSE);
    element_clear(neg_r);
    element_clear(r);
